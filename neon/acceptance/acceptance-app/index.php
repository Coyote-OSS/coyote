    ...
    ->withRouting(function (): void {
        Route::middleware([
            ...
            Route::get('/assets/{filename}', function (string $filename): Response {
                $jobBoard = new Neon\View\JobBoard('/');
                return response(File::get($jobBoard->assetPath("/assets/$filename")));
            });
        });
    })
...
function neonView(JobBoard $jobBoard): string
{
    return <<<html
        <html>
        <head>{$jobBoard->htmlMarkupHead()}</head>
        <body>
        <div id="jobBoardRoot">{$jobBoard->htmlMarkupBody()}</div>
        </body>
        </html>
        html;
}
